...
final class PhotoLibraryService {
	...
	func requestAuthorization(_ success: @escaping () -> Void, failure: @escaping (_ err: String) -> Void ) {
		...
		if status == .authorized {
			...
			// Ask for permission every time we do not get the authorization
			let callback = {(status:PHAuthorizationStatus) in
			...
			if #available(iOS 14.0, *) {
				//in iOS14, we can specify that we only want to save, this way we can avoid the "limited" case
		...
		//Disabled because we stay stuck if the user does not modifies the settings:
		/*
