...
//                        let mime_type = self.mimeTypes[url.pathExtension.lowercased()]
	...
	func authorizationStatusAsString()->String{
		...
		if #available(iOS 14.0, *) {
			...
			if status == .limited {
				...
				if addStatus == .authorized {
					// consider that being denied readWrite and allowed addOnly is the same as limited readWrite
					// (in both cases, we are going to use saveImageWithoutAlbum() )
					return "limited"
				}
			}
		}else{
	...
	//Checks the authorization status, and requests it if necessary. Sends back the final status as string once done.
	func requestAuthorization(_ success: @escaping (_ status:String) -> Void) {
		let status = authorizationStatusAsString()
		print("requestAuthorization - got", status)
		if status == "authorized" || status == "limited" {
            success(status)
		}else{
			PHPhotoLibrary.requestAuthorization{ _ in
				print("requestAuthorization - after request, got", status)
				success(self.authorizationStatusAsString())
			}
		}
    }

	// Assume the caller checked the permissions himself by calling requestAuthorization
	// => we should have "authorized" or "limited" auth status
    func saveImage(_ url: String, album: String, completion: @escaping (_ error: String?)->Void) {
		let status = authorizationStatusAsString()
		if #available(iOS 14.0, *) {
			if status == "limited" {
				print("PhotoLibraryService.saveImage with limited access")
				
				//user did not give us full access to the library, so we won't be able to save the image to an album
				// => fall back on basic saving (image will end up in Photos > recent, but not in the specified album)
				return saveImageWithoutAlbum(url, completion: completion)
			}
		}
		
		//save to album
		print("PhotoLibraryService.saveImage to album", url, album)
		//convert url to Data
		let sourceData: Data
		do {
		...
		func saveImage(_ photoAlbum: PHAssetCollection) {
			assetsLibrary.writeImageData(toSavedPhotosAlbum: sourceData, metadata: nil) { (assetUrl: URL?, error: Error?) in
				if error != nil {
					completion("Could not write image to album: \(error)")
					return
				}
				guard let assetUrl = assetUrl else {
					completion("Writing image to album resulted empty asset")
					return
				}
	...
	//For iOS 14 with limited access: direct saving to Photos without using an album
	var uiImageWriteToSavedPhotosAlbumCallback:((_ error: String?)->Void)? = nil
	func saveImageWithoutAlbum(_ url: String, completion: @escaping (_ error: String?)->Void) {
		...
		guard let imageUrl = URL(string: url) else {
			completion("failed to parse url")
			return
		}
		if let uiImage = UIImage(contentsOfFile: imageUrl.path) {
			self.uiImageWriteToSavedPhotosAlbumCallback = completion
	...
	@objc func imageSaved(_ image: UIImage, didFinishSavingWithError error: NSError?, contextInfo: UnsafeRawPointer) {
		print("PhotoLibraryService.imageSaved -", error?.localizedDescription ?? "ok", "callback", self.uiImageWriteToSavedPhotosAlbumCallback != nil)
		if let error = error {
			uiImageWriteToSavedPhotosAlbumCallback?(error.localizedDescription)
		} else {
			uiImageWriteToSavedPhotosAlbumCallback?(nil)
		}
	}
